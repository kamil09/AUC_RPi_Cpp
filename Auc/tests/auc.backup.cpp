...
{ 
    ...
    int lowerHUE = 34, upperHUE = 58;
    int lowerSATURATION = 74, upperSATURATION = 255;
    int lowerVALUE = 98, upperVALUE = 255;
...
//            drawContours( drawing,  contours, i_cont, Scalar(125, 125, 250), 2 ); 
		...
		if((abs(dX) > dXDiff ) || (abs(dY) > dYDiff ))
		{
			...
			laserOFF();	
			double dxD = (double)(dX)/25;
			double dyD = (double)(-dY)/25;
			printf("kąt X: %f ; kąt Y: %f\n",dxD,dyD );
			int status = move(dxD, dyD, 3.0);
